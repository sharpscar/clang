...
#define MAX_BOOKS 10000           // 도서 최대 등록 수
...
void *client_handler(void *arg) {
    ...
    /*로그인 결과를 클라이언트 소켓에 담아 전송!*/
    write(client_socket, &result, sizeof(int));

    /*로그인이 된 상태에서 명령이 로그인인경우*/
    if (result == 1 && strcmp(cmd, "login") == 0) {
        while (1) {
            char action[16];
            read(client_socket, action, sizeof(action));  // get/post 처럼 서버에서 클라 요청 읽는다.
            action[strcspn(action, "\n")] = '\0'; // 액션에서 개행문자가 나오기 전까지 나오는


            if (strcmp(action, "search") == 0)
            {
                char key[50], val[150];
                Book *p_found = malloc(sizeof(Book) * MAX_BOOKS);
                memset(p_found, 0, sizeof(Book) * MAX_BOOKS); //? 검색했을때 굳이 10000개만큼 메모리를 ?
                Book *tmp;

                read(client_socket, key, sizeof(key));  //title or author
                key[strcspn(key, "\n")] = '\0';
                read(client_socket, val, sizeof(val));  //검색어
                val[strcspn(val, "\n")] = '\0';
                int count = search_books(key, val, p_found);
                write(client_socket, &count, sizeof(int));
                write(client_socket, p_found, sizeof(Book) * count);
            ...
            {
                char isbn[200];
                read(client_socket, isbn, sizeof(char)); //클라이언트 소켓정보에 해당하는 ispn 버퍼를 받아 요청 
...
int main() {
    ...
    while (1) {
        ...
        *client_socket = accept(server_fd, (struct sockaddr *)&address, &addrlen); // 연결 수락
        ...
        pthread_t tid;
        pthread_create(&tid, NULL, client_handler, client_socket);  // 쓰레드 생성 네트워크의 main함수
        pthread_detach(tid);  // 쓰레드 자원 자동 해제
    }
    close(server_fd);
    return 0;
}
/* 클라이언트 1, 2 동시에  작동할수 있기 때문에
    변수는 하난데 서로다른 쓰레드에서 
*/
