    ...
    if (result == 1 && strcmp(cmd, "login") == 0) {
        while (1) {
            ...
            if (strcmp(action, "search") == 0)
            {
                ...
                {
                    printf("%d, %d, %s, %s\n", count, i, found[i].title, found[i].author);
                }
                write(client_socket, &count, sizeof(int));
                for (int i = 0; i < count; i++)
                {
                    write(client_socket, &found[i], sizeof(Book));
                }
            }
            // 클라이언트 도서조회 요청 끝
            ...
            if (strcmp(action, "add") == 0)
            {
                ...
                //요청받은것을 함수를 호출하여 도서를 추가했다.
                result = add_book(b);
                //도서 추가결과를 클라이언트에 응답한다.
                write(client_socket, &result, sizeof(int));
            } 
