...
int main() {
    ...
    // 로그인 or 회원가입 명령 전송  매개변수 (소켓, 전송할데이터, 데이터크기)
    ...
    while (1) {
        ...
        if (strcmp(cmd, "search") == 0) {
            ...
            int count;
            /**구조체 배열도 이런식으로 초기화가 되는가?  */
            Book results[MAX_BOOKS] = {0}; 
            
            // 검색 결과에 대해 사용자에게 노출 count ->검색된도서수
            read(sock, &count, sizeof(int));
            if (count>0)
            {
                for (int i = 0; i < count; i++)
                {
                    read(sock, &results[i], sizeof(Book));
                }
                /**왜 반복문을 나누었을까??? 둘다 i< count 만큼 반복  */
                for (int i = 0; i < count; i++)
                {
        ...
        //클라이언트[사서, 관리자]가 도서를 추가하는 명령을 함
            ...
            //서버에 도서를 추가 요청 보냄 책 1권 정보
            send(sock, &b, sizeof(Book), 0);
            // 서버에 도서 추가 요청후 응답결과를 사용자에게 보인다.
            read(sock, &result, sizeof(int));
