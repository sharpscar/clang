...
#define GREEN "\033[32m" // 초록색
#define CYAN "\033[36m" // 초록색
#define RESET "\033[0m" // 색상 초기화

#define MAX_MEMBERS 100
#define PAGE_SIZE 10     // 한 페이지당 표시할 파일 수
...
void show_my_info(int login_index); //내정보 확인하기 프린트
...
void print_member_page(member *arr, int total_count);  //정렬하여 10개씩 출력
void admin_member_page(member *arr, int total_count);
...
//텍스트기능 관련 함수
void delete_a_article_from_admin_articles(int del_index);  //삭제
void store_to_admin_article_struct();  //구조체로 변환
void store_to_article_struct();  //저장
void write_a_admin_article();  //관리자 글쓰기 기능
article get_input_for_write_article();  
void show_article_list();  //목록보기
int write_user_article_with_number(const char *user_id);  //글쓰기파일을 생성
void show_paging();  //페이징
int getTotalLine(char *name);  //줄 개수 세는 함수
int do_mkdir(const char *pasPathName, mode_t mode);
void main_send(member *a, int login_index);                                                     // ******************************수정했나?
void text_list(const char *folder_path);                                                     // ******************************수정했나?
void text_list_2(const char *folder_path, const char *user_id, const char *send_id);         // ******************************수정했나?
...
int main(){
    ...
    while(1){
        ...
        else if(main_menu_choice == 2){  //회원가입
        ...
        else if(main_menu_choice != 1 && main_menu_choice !=2){
...
int print_main_menu(){
    ...
    int choice;
        ...
        printf(GREEN"┌──────────────────────────────────────────────────────────┐\n");
        puts("│                                                          │ ");
        puts("│   ¶ 인공ズl능개발팀™ 회원입출력 ㅍr일관己l 프로ュ램 ⊙ •  │");
        puts("│                                                          │ ");
        puts("└──────────────────────────────────────────────────────────┘");
        printf("\n"RESET);
        puts(" 1: 로그인 \t\t2: 회원가입");
...
void main_menu2(int login_index) {
    ...
    printf("사용자 : %s", new_mem[login_index].name);
    while (1) {
        system("clear");
        puts("");
        puts("");
        ...
        switch (choice) {
            case 'w':
                ...
                break;
            case 'a':
                char folder_path[100];                                    // ******************************수정했나?
                snprintf(folder_path, sizeof(folder_path), "./server/%s", login_id);    // ******************************수정했나?
                text_list(folder_path);
                break;                                                   // ******************************수정했나?
                // ******************************수정했나?

            case 't':
                main_send(new_mem, login_index);         // new_mem은 회원 등 login_index 는 가져가서 안에서 id구함 // ******************************수정했나?
                break;
            case 'f':
            ...
            while(main_menu2_choice !=1 || main_menu2_choice !=2);
...
int join_id_check(member *a){
    ...
    while(1){
        ...
        if(strlen(join_id) > 4 && strlen(join_id) <= 20){// ID 다섯글자이상. 20자 이하.
...
int save_phonenum() {
    ...
    while(1){
        ...
        if ((num_count == -1)||(digit_count !=11))
        {
            printf("전화번호는 숫자와 '-'만 입력할 수 있습니다. 11자리 입력해주세요\n");
        }
        else if((num_count == strlen(phonenum))&&(digit_count == 11)){
...
void retrieve_id_from_member(int* found_account, char id_account[]) {
    ...
    printf("\n");
    printf("총 %d개의 아이디를 찾았습니다.\n", cnt);
    printf("\n");

    for (int i = 0; i < cnt; i++) {
        printf("[이름]   [아이디]   [나이]      [전화번호]\n");
        printf("%s    %s     %d       ",temp[i].name,temp[i].id,temp[i].age);
        masking_user_phonenum(temp, i);
    }
    printf("\n   > > > 메인 화면으로 돌아갈게요! (아무 숫자나 입력) > > >\n");
    scanf("%d",&check);
...
void print_member_page(member *arr, int total_count) {
    ...
    while (1) {
        ...
        for (int i = start; i < start + 10 && i < total_count; i++) {
            printf("%s    %s     %d       ",arr[i].name,arr[i].id,arr[i].age);
...
void admin_member_page(member *arr, int total_count) {
    ...
    while (1) {
        ...
        printf("[이름]   [아이디]   [나이]      [전화번호]\n");
        for (int i = start; i < start + 10 && i < total_count; i++) {
            printf("%s    %s     %d          %s\n",arr[i].name,arr[i].id,arr[i].age,arr[i].phone_num);
        }
...
int write_user_article_with_number(const char *user_id) {                  // 함수 통째 ******************************수정했나?         
    char folder_path[100];
    char file_path[100];
    char title[100], content[500];
    int post_number = 1;
    ...
    strftime(datetime, sizeof(datetime), "%Y%m%d_%H%M%S", t); // 현재 시간 포매팅
    ...
    // snprintf(filepath, sizeof(filepath), "./server/%s/%s_[New]_%s.txt", a.writer_id, datetime, a.article_title); // 최종 파일 경로 생성

    // 3. 새 글 파일 경로 만들기
    snprintf(file_path, sizeof(file_path), "%s/%s_%s.txt", folder_path, user_id, datetime);    //datetime
...
// 첫번째는 나(보낸이)두번째는 받는 사람 
void write_user_article_with_number_send(const char *user_id, const char *send_id) {     // 함수 통째 ******************************수정했나?         
    char folder_path[100];
    char file_path[100];
    char title[100], content[500];
    int post_number = 1;
    ...
    // 1. 유저 폴더 경로 만들기
    snprintf(folder_path, sizeof(folder_path), "./server/%s", send_id);
    ...
    while ((entry = readdir(dir)) != NULL) {
        // 글 파일명: user_id_숫자.txt
        if (strncmp(entry->d_name, send_id, strlen(send_id)) == 0) {
            post_number++;
        }
    }
    closedir(dir);
    ...
    // 3. 새 글 파일 경로 만들기
    snprintf(file_path, sizeof(file_path), "%s/[NEW]_%s_[보낸이:%s]_%s.txt", folder_path, send_id, user_id, datetime);


    // snprintf(file_path, sizeof(file_path), "%s/%s_%d_[보낸이:%s][NEW].txt", folder_path, send_id, post_number, user_id);

    // snprintf(file_path, sizeof(file_path), "%s/%s_%s.txt", folder_path, user_id, datetime); 

    // snprintf(filepath, sizeof(filepath), "%s/%s_[New]_%s.txt", folder, datetime, title); // 최종 파일 경로 생성
  ...
  // 첫번째는 나(보낸이)   두번째는 받는 사람                 // 아래 함수 통째 ******************************수정했나? 
void write_user_article_with_number_send_2(const char *firepath, const char *user_id, const char *send_id) {      
    char folder_path[100]; 
    char file_path[100];
    char title[100], content[500];
    int post_number = 1;
...
void main_send(member *a, int login_index) {                 // 유저넘초이스가 login index로 들어옴 !!!!!!!!
   
    system("clear");

    char choice;
    char send_id[50];                      // 들어온 구조체는   id, pw, name, age, phonenum
    char user_id[50]; 
    
    strcpy(user_id, a[login_index].id);

    int flag_id = 0;
    // int user_num = 0;

    
    char title[100], filepath[200], folder[150], datetime[50], line[300];
    time_t now = time(NULL);
    struct tm *t = localtime(&now);
    strftime(datetime, sizeof(datetime), "%Y%m%d_%H%M%S", t); // 현재 시간 포매팅
    ...
    while(1){
        puts("받는 사람의 아이디를 입력하세요: ");
        scanf("%s",send_id);         // 입력받은 id를 login_id에 넣어서
    
        for(int i = 0; i < 100; i++){
            if (strcmp(send_id,a[i].id)==0){                      
                flag_id = 1;                      // 같은놈 있으면 flag로 1떠서 있다고 알림
                puts("전송 가능한 아이디입니다.");
                }
            }
        ...
        getchar();                                                             ///////////////////////// 이거도 수정함 !!!!!!!!!!!!!!
            ...
            switch (choice) {                
                case 'w':                                                          ///////////////////////// 이거도 수정함 !!!!!!!!!!!!!!
                    ...
                    // write_a_article(new_mem, login_index);  // 유저 ID 넘기기           write_user_article_with_number_send(const char *user_id, const char *send_id)
                    write_user_article_with_number_send(user_id, send_id);
                    break;
                ...
                case 't':
                    ...
                    // text_list(folder_path);
                    text_list_2(folder_path, user_id, send_id);
                    break;
...
int cmpfunc(const void *a, const void *b) {                // text_list관련 최신 정렬 함수 
    return strcmp(*(const char **)b, *(const char **)a);
}

void text_list(const char *folder_path) {  // 로그인한 아이디의 폴더주소를 받아 들어옴
    
    char *files[200];                     // strdup을 통해 파일명을 담음
    int count = 0, page = 0;              // 파일 개수, 현재 페이지
    char input[10];
    ...
    while ((entry = readdir(dir))) {
        ...
        struct stat st;
        if (stat(fullpath, &st) == 0 && S_ISREG(st.st_mode)) {               // stat 통해 일반 파일만 추가????
    ...
    qsort(files, count, sizeof(char *), cmpfunc);              // quick sort(배열, 원소수,  원소하나크기, 비교수행함수)
    ...
    while (1) {
            ...
            if (file_count >= 90 && file_count < MAX_FILES) {         // 파일 90개 이상시 경고

                printf("경고 파일 90개 이상 / 100개가 초과시 오래된 순으로 삭제\n\n");

            }
            else if(file_count >= MAX_FILES) {
                printf("파일 100개 초과로 인한 자동 삭제\n\n");
            }
            ...
            printf("\ne.다음 / q.이전 / (번호)r.읽기 / (번호)d.삭제 / z.돌아가기\n> ");
            scanf("%s", input);
    
            if (!strcmp(input, "e") && end < count) {
                page++;
            } 
            
            else if (!strcmp(input, "q") && page > 0) {
                page--;
            } 
                ...
                int num = atoi(input);                     // 앞 숫자 추출해서 파일 인덱스를 계산(atoi)
                char cmd = input[strlen(input) - 1];       // 마지막 문자 확인 (r, d) // 마지막 문자를 추출해서 r이면 읽기, d면 삭제
                char cmd2;
                ...
                if ((cmd >= '0') && (cmd <= '9')) {
                    getchar();
                    printf("%c번 (r) : 파일 읽기 / (d) : 삭제 \n", cmd);
                    scanf("%c", &cmd2);
                }
    
                char filepath[300];
                snprintf(filepath, sizeof(filepath), "%s/%s", folder_path, files[num - 1]);
                ...
                if ((cmd == 'r') || (cmd2 == 'r')) {
                    ...
                    fclose(fp);
                     ...
                     // [NEW] 읽으면 없애는 로직
                    if (strstr(files[num - 1], "[NEW]_")) {              // [NEW]가 있으면
                        char newname[250];                                 
                        strcpy(newname, files[num - 1]);                 // newname 변수에 files[num -1]을 넣고 
                        char *p = strstr(newname, "[NEW]_");             // p포인터에 ststr로  [NEW]위치 넣고
                        if (p) memmove(p, p + 6, strlen(p + 6) + 1);     // memmove로 땅겨
    
                        char old_path[300], new_path[300];
                        snprintf(old_path, sizeof(old_path), "%s/%s", folder_path, files[num - 1]);   
                        snprintf(new_path, sizeof(new_path), "%s/%s", folder_path, newname);
                        rename(old_path, new_path); //                   파일명 변경
    
                        free(files[num - 1]);
                        files[num - 1] = strdup(newname);
                    }
    
                } else if ((cmd == 'd') || (cmd2 == 'd')) {
                    printf("정말 삭제하시겠습니까? (y/n): ");
                    char confirm;
                    scanf(" %c", &confirm);
                    if (confirm == 'y') {
                        remove(filepath);                        // 파일 삭제 // 목록에서 제거
                        printf("삭제 완료\n");
                        free(files[num - 1]);
                        for (int i = num - 1; i < count - 1; i++) {
                            files[i] = files[i + 1];                       // 배열 당기기
                        }
                        count--;
                    }
                } else {
                    printf("다시 입력해주세요.(ex : 1r, 2d)\n");
                }
            }
        }
    
        for (int i = 0; i < count; i++) {
            free(files[i]);                                       // 동적 메모리 해제
        }
    }
    ...
    void text_list_2(const char *folder_path, const char *user_id, const char *send_id) {  // 로그인한 아이디의 폴더주소를 받아 들어옴
                    ...
                    if ((cmd >= '0') && (cmd <= '9')) {
                        ...
                        system("clear");
                            ...
                            if (confirm == 'y') {
                                ...
                                write_user_article_with_number_send_2(filepath, user_id, send_id);
