...
void delete_a_article_from_articles(int index);
...
{  
    ...
    //     //new_mem[i].id
    //     result = do_mkdir(new_mem[i].id, 0755);
    //     if(0>result)
    //     {
    //         printf("디렉토리 생성 실패\n");
    //     }else{
    ...
    printf("글의 갯수 아마 15개 %d", articles_cnt);
    for(int i=0; i<articles_cnt; i++)
    {
        ...
        {   
            printf("글 아이디 %d", articles[i].article_id);
            printf("단순 글작성 작성자 아이디로 디렉터리 생성및 파일생성\n");
        ...
        {
            printf("글 아이디 %d", articles[i].article_id);
            printf("받는사람 아이디로 디렉터리 생성및 파일생성\n");
            
            int result;
            result = do_mkdir(articles[i].reciever_id, 0755);
