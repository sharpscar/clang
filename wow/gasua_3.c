...
void print_member_page(member *arr, int total_count);
void main_send(member *a, int login_index);
void write_user_article_with_number_send(const char *user_id, const char *send_id);

void text_list(const char *folder_path);
void text_list_2(const char *folder_path);
    ...
    while (1) {
        ...
        switch (choice) {
            ...
            case 'a':
                ...
                snprintf(folder_path, sizeof(folder_path), "./server/%s", login_id);
...
void write_user_article_with_number_send(const char *user_id, const char *send_id) {       // 첫번째는 나(보낸이)   두번째는 받는 사람
    char folder_path[100];
    char file_path[100];
    char title[100], content[500];
    int post_number = 1;
...
void text_list(const char *folder_path) {  // 로그인한 아이디의 폴더주소를 받아 들어옴
    ...
    void text_list_2(const char *folder_path) {  // 로그인한 아이디의 폴더주소를 받아 들어옴
    ...
    while (1) {                                         
        ...
        getchar();                                                             ///////////////////////// 이거도 수정함 !!!!!!!!!!!!!!
            ...
            switch (choice) {                
                ...
                case 'a':
                    show_article_list();                // 텍스트파일 읽어냄 login_id는 의미없음 현재상태에서는
                    break;
