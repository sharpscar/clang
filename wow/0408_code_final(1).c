    ...
    while(1){
        ...
        if(admin_menu_choice == 't'){  //텍스트보내기
            ...
            do{
                getchar();
                system("clear");
                admin_menu_choice == log_out_or_exit();
            ...
            while(admin_menu_choice !=1 || admin_menu_choice !=2);
        }
        else{
            printf("다시 입력하세요.\n");
        }
...
int write_user_article_with_number(const char *user_id) {
    char folder_path[100];//폴더 경로 저장해서 담을 변수
    char file_path[100];  //파일 경로 저장해서 담을 변수
    char title[100], content[500];  // 글 제목, 글 내용
    int post_number = 1;  // 글번호의 시작 기본 값

    // 1. 유저 폴더 경로 만들기
    snprintf(folder_path, sizeof(folder_path), "./server/%s", user_id);  

    // 2. 폴더 열고 유저 글 파일 개수 세기
    DIR *dir = opendir(folder_path); //폴더를 엽니다.
    if (dir == NULL) {   //열기 실패하면
        perror("폴더 열기 실패");
        return 0;
    }

    struct dirent *entry;  //디렉토리 안의 파일이나 폴더를 답을 구조체 포인터
    while ((entry = readdir(dir)) != NULL) {
        // 글 파일명: user_id_숫자.txt
        if (strncmp(entry->d_name, user_id, strlen(user_id)) == 0) {  //파일명이 유저 아이디와 같은지 확인
            post_number++;  //일치하면 카운트 증가시킴
        }
    }
    ...
    // 5. 파일에 저장
    FILE *fp = fopen(file_path, "w");  //쓰기모드로 파일 경로 열어서
    if (fp == NULL) {
        perror("파일 저장 실패");
        return 0;
    }
    ...
    fprintf(fp, "== %s ==\n제목: %s\n내용: %s\n", time_str, title, content);  //내용 저장하기
    fclose(fp);   //파일 닫기
