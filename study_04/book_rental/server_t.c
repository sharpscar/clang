    ...
    if (result == 1 && strcmp(cmd, "login") == 0) {
        ...
        while (1) {
            ...
            else if (strcmp(action, "2") == 0) // 모든계정관리
            {
                ...
                char subcmd[10];
                read(client_socket, subcmd, sizeof(subcmd));

                if(strcmp(subcmd, "1") == 0) {  // 계정 수정
                    int user_num;
                    User modified_user;
                    read(client_socket, &user_num, sizeof(int));
                    read(client_socket, &modified_user, sizeof(User));

                    // 실제 배열 인덱스로 변환 (display_index + 1)
                    user_num += 1;

                    if(user_num >= 0 && user_num < user_count) {
                        // 기존 ID와 비밀번호는 유지
                        strcpy(modified_user.id, Users[user_num].id);
                        strcpy(modified_user.pw, Users[user_num].pw);
                        modified_user.msc = Users[user_num].msc;

                        Users[user_num] = modified_user;
                        ...
                        if(fp) {
                            ...
                            for(int i = 0; i < user_count; i++) {
                                ...
                                cJSON_AddStringToObject(user, "password", Users[i].pw);
                                cJSON_AddStringToObject(user, "name", Users[i].name);
                                cJSON_AddNumberToObject(user, "age", Users[i].age);
                                cJSON_AddStringToObject(user, "phone", Users[i].phone);
                                cJSON_AddStringToObject(user, "addr", Users[i].addr);
                                cJSON_AddNumberToObject(user, "messagecount", Users[i].msc);
                ...
                else if(strcmp(subcmd, "2") == 0) {  // 계정 추가
                    User new_user;
                    read(client_socket, &new_user, sizeof(User));
